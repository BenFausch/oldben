...
if(!class_exists('soundCloudUltimate')) 
{
	class soundCloudUltimate 
	{
		...
		function soundcloud_ultimate_sc_handler($atts) {	
			...
			if($track == "no track"){
				echo '<div class="error fade" style="color:red;"><p><strong>SoundCloud Ultimate Error: You need to specify a track URL in your shortcode.</strong></p></div>';
				return;
			}
		...
		function get_soundcloud_widget_code($track, $auto_play=false) {
			...
			try {
				...
				echo '<div style="color:red;"><p><strong>SoundCloud Ultimate Error: Could not display your SoundCloud track - Error code ('.$e->getHttpCode().').</strong></p></div>';
				return;				
			}
			...
			try {
				...
				if ($e->getHttpCode() == '404') {
					echo '<div style="color:red;"><p><strong>SoundCloud Ultimate Error: The track you specified in the shortcode does not exist in your account.</strong></div>';
					return;
				}
				echo '<div style="color:red;"><p><strong>SoundCloud Ultimate Error: Could not display your SoundCloud track - Error code ('.$e->getHttpCode().').</strong></p></div>';
				return;
